...
const fetchMenu = async () => {
    try {
        ...
        const data = await $fetch(`${config.public.apiBase}/menu/${menuid}`, {
            headers: { Authorization: `Bearer ${token}` }
        })

        form.mname = data.mname
        form.description = data.description || ''
        form.cookhours = Math.floor(data.cooktime / 60)
        form.cookminutes = data.cooktime % 60
        selectedCategory.value = data.categoryname
        imagePreview.value = data.cover_image || null

        form.ingredients = data.ingredients?.length
            ? data.ingredients.map(i => i.ingredient_name)
            : ['']

        form.steps = data.steps?.length
            ? data.steps.map(s => ({ step: s.step }))
            : [{ step: '' }]

    } catch {
        errorMsg.value = 'ไม่สามารถโหลดข้อมูลเมนูได้'
    } finally {
...
onMounted(async () => {
    if (!menuid) return router.push('/app/main')
...
const confirmDelete = async () => {
    ...
    try {
        const token = localStorage.getItem('token')
        await $fetch(`${config.public.apiBase}/menu/${menuid}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
        })
        router.push('/app/main')
    } catch {
        errorMsg.value = 'ไม่สามารถลบเมนูได้'
    }
}
...
<template>
    <div style="font-family: 'Bai Jamjuree'">
        ...
        <div v-if="pageLoading" class="min-h-screen flex items-center justify-center">
            <svg class="animate-spin w-10 h-10 text-green-600" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
            </svg>
        </div>

        <div v-else class="min-h-screen bg-gray-100 p-10">
