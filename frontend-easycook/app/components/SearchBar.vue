    ...
    <section class="flex flex-col items-center justify-center px-4 py-8 gap-8">
        <div class="flex gap-3 items-center relative">
            ...
            <div ref="dropdownRef" class="relative">
                ...
                <!-- Dropdown Panel -->
                <div v-if="isOpen" class="absolute top-12 right-0 bg-white border border-gray-200 rounded-2xl shadow-lg z-50 p-6 w-175">
                    <div class="mb-4">
                    ...
                    <div class="flex gap-4">
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button
                                    v-for="cat in mainCategories" :key="cat"
                                    :class="[
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button
                                    v-for="cat in cookingCategories" :key="cat"
                                    :class="[
                                        'px-4 py-1.5 rounded-full text-sm border transition-colors cursor-pointer',
                                    ...
                                    @click="toggleCategory(cat, 'cooking')"
                                >
                                    {{ cat }}
                                </button>
                            </div>
...
const toggleCategory = (cat, type) => {
    if (type === 'main') {
        ...
        if (idx === -1) selectedMain.value.push(cat)
        else selectedMain.value.splice(idx, 1)
    } else {
        const idx = selectedCooking.value.indexOf(cat)
        if (idx === -1) selectedCooking.value.push(cat)
        else selectedCooking.value.splice(idx, 1)
    }
}
